...
fo=open('AmpliTable_del_Five.txt','w')
    ...
    for key2 in Id2InfoNum:
        ...
        for key,value in sorted(Id2InfoNum[key2].items()):
            if(key<=5):
                count_min+=value
            if(key==6):
                fo.write(bam_file_prefix+'\t'+key2+'\t<=5D\t'+str(count_min)+'\t'+str(Id2dep[key2])+'\n')
            if(key>=6 and key<=80):
                count+=value
                if(key%5==0):
                    fo.write(bam_file_prefix+'\t'+key2+'\t'+str(key-4)+'-'+str(key)+'D'+'\t'+str(count)+'\t'+str(Id2dep[key2])+'\n')
                    count=0
            if(key>80):
                count_large+=value
            if(key==499):
                fo.write(bam_file_prefix+'\t'+key2+'\t>80D\t'+str(count_large)+'\t'+str(Id2dep[key2])+'\n')
    ind_fi+=1
